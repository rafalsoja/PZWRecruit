...
* **Automated Announcements**: Sends guild recruitment messages to specified custom or global channels at set intervals.
* **Multi-Officer Sync**: Synchronizes broadcast timestamps across all recruiting officers via hidden guild messages (`GUILD` addon channel) to prevent duplicate spam.
* **In-Game Options Panel**: Easily configuration via UI panel with status tracking (last send time, total messages sent, and last sender).
